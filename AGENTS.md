...
- Final NotebookLM validation is a count-and-identity invariant, not a visual spot check. Confirm `TXT sources = NotebookLM sources = accepted queue items = completed cards = expected card titles = verified ledger entries = downloaded assets = linked chapters`; confirm no `正在生成語音摘要` remains, no duplicate/missing chapter number or marker exists, every asset passes metadata probing, last-packet duration, and full-decode checks, every `audio.src` maps to its chapter, and the reader UI exposes a separate NotebookLM Podcast play/download entry. Do not report completion while any equality or identity check fails.
- Use a four-layer audio acceptance model and never substitute one layer for another: (1) metadata/identity, (2) last-packet duration, (3) full local decode, and (4) deployed playback. After deployment, require every production audio URL to return HTTP 200 with an audio content type and `Content-Length` exactly matching the validated local asset, then perform a five-second remote decode seek for every chapter at 120 seconds or the chapter midpoint when shorter. Run `npm run validate:notebooklm-audio -- <slug> <production-base-url> remote-seek` and require `status: passed` with `productionRemoteSeekChecked: true` before declaring the audio release complete.
